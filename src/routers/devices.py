...
async def patch_device(patch_device: schemas.DevicePatch):
    try:
        ...
        return schemas.DeviceFrontend(
            ...
            last_seen=str(patched_device.last_seen) if patched_device.last_seen else None,
            last_update_sent=str(patched_device.last_update_sent) if patched_device.last_update_sent else None,
        )
    except NoResultFound as e:
        raise HTTPException(status_code=500, detail=str(e.args[0]))
    except Exception as e:
        logger.error("Error patching device with mqtt id %s: %s", patch_device.mqtt_id, e)
