...
def test_crud_get_tags_by_ids_returns_tags(populated_test_db):
...
def test_crud_create_tag_creates_tag(empty_test_db):
    tag = crud.create_tag("New Tag")
    assert tag.name == "New Tag"
    assert crud.get_tag_by_name("New Tag").name == "New Tag"


def test_crud_delete_tag_deletes_tag(populated_test_db):
    crud.delete_tag(1)
    assert crud.get_tag_by_id(1) is None


def test_crud_patch_tag_patches_tag(populated_test_db):
